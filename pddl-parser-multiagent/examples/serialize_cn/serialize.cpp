...
**
*************************************************************************************************** 
Before you dive into the code, you should keep the following steps in mind.	Note that our code is restricted to very specific cases as per our ICAPS paper, so it does not handle all the scenarios, however, it is very easy to make changes in the code according to any specific requirements.  	
...
Relevant for the ICAPS paper. 
	...
	2. Keep all the agents together in the activity parameter list. Since it is an extension over CJR's formalism and syntax, the prime acting agent would never appear in the parameter list,  
	see their action schema for more details on this. 
		...
		(b) Irrespective of a object-type Truck or Plane use AGENT as the code has string comparisons, especially for a collaborative action.
		(c) Since all the actions' parameters are purposely being kept symmetric, normal string comparisons w.r.t. the collaborative-action's name and its components' names would do. 
			In other case we need to go over all the parameters of component actions too, to find out whether a SA action is a component of a collaborative action.
	
	3. The name of a collaborative action should have "ACTIVITY" string in it. Also, you need to specify more than one agent. See our domains for example.  
	
	4. It splits blindly, which means, to have correct splits of a Joint activity, make sure that there are no predicates specifying other agents in the effect or precondition. A little more 
	work is needed to resolve this limitation. 
	
	5. TODO during translation an action cannot be public or private. So, remove IN-JOINT from the preconditions in the planner's code -- whichever planner you are using.
	
	6. Simplification - the single-agent action name will appear in the name of a collaborative action, for example, push-box and activity-push-box.
	
	7. As there is some problem in the MA-Parser (in upf-map, unresolved), during parsing a problem file, it always skips the first private object. Therefore, always have a dummy object in each
	problem file, for an example, "dummy-pr - object", we also use this.
	
	8. It should capture POS- or NEG- for a predicate only if its param set is superset of node param set, e.g., V1->params   
	
	9. Since, the number of agents is not known beforehand, so, unlike CJR, 'inf' does not work. We need to give an appropriate number in the node for upper bounds.
   	
   	10. Don't have a check on a node having inappropriate bounds, and it has a collaborative action in its action list. 
   	
Note that, in the description, our paper considers all actions as grounded actions, so that the explanations become easy. The code is based on non-grounded operators. However, relevant modifications 
...
The changes below are part of our submitted journal article.
   	...
   	12. We have changed the interpretation now. Now, we have a stronger notion to define a well-formed multi-action instead of well-defined that has some potential weakness as pointed out in our
   	ICAPS paper.  
   	
   	13. However, the current interpretation does not affect our domains' compilation.  
   	
   	14. Note that, the code for point 2(d), you need to manually change AND to OR. For example, (NOT (AND () () ()) ) to (OR (NOT ()) (NOT ()) ), the parser does not support it, so cannont 
		...
		{				
			...
			{ 			
				for( unsigned l = 0; l < probably_legal_a->deleteEffects().size(); ++l ) 
				{
					...
					{
						ambiguous1 = doTheyTargetTheSameObjectSubset( n->params, legal_a->addEffects()[k]->params, legal_a->params, probably_legal_a->deleteEffects()[l]->params,
			...
			if( !ambiguous1 ) 
			{
				...
				{ 			
					for( unsigned l = 0; l < probably_legal_a->addEffects().size(); ++l ) 
					{
						...
						{	
							ambiguous2 = doTheyTargetTheSameObjectSubset( n->params, legal_a->deleteEffects()[k]->params, legal_a->params, probably_legal_a->addEffects()[l]->params,
												probably_legal_a->params );
							if( ambiguous2 ) 
							{
								break;												
